...
def.calc.reaeration <- function(
  ...
  for(i in seq(along = outputDF$eventID)){
   ...
   # s4peakLoc <- reaRate::def.calc.peakTime(loggerDataIn = condDataS4,
    ...
    outputDF$S1CentroidTime[i] <- s1peakLoc$centroidTime } else {
      next 
      }
    
    if(!is.null(s4peakLoc$centroidTime)& length(s4peakLoc$centroidTime) > 0) {
    outputDF$S4CentroidTime[i] <- s4peakLoc$centroidTime } else {
      next
    }
    ...
    if(!is.null(s4peakLoc$centroidTime) & length(s4peakLoc$centroidTime) > 0) {
      if(!is.na(currExpStartTime)) {
        outputDF$releasetoCentroidTravelTime[i] <- difftime(s4peakLoc$centroidTime,
                                                 currExpStartTime,
                                                 units = "secs") }} else {
          next}
    ...
    #  invisible(dev.new(noRStudioGD = TRUE, width=12, height=7))
      ...
      points(condDataS4$dateTimeLogger[condDataS4$dateTimeLogger > currExpStartTime & condDataS4$dateTimeLogger < s4peakLoc$endPlotTime],
             s4YData,
             col = "blue")
      try(abline(v = s1peakLoc$centroidTime, col="black"))
      try(abline(v = s4peakLoc$centroidTime, col = "blue"))
    ...
    outputDF$reaRateSF6_nominal[i] <- outputDF$lossRateSF6[i] * outputDF$nominalVelocity[i] * -1 * 86400# m^-1 * m/s * -1 for negative slope and 86400 for number of seconds in a day
    #loss per day
    
    outputDF$reaRateSF6release_nominal[i] <- outputDF$lossRateSF6[i] * outputDF$releaseVelocity_nominal[i] * -1 * 86400# m^-1 * m/s * -1 for negative slope and 86400 for number of seconds in a day
    #loss per day
    
    #Calculate the gas transfer velocity for oxygen
    outputDF$reaRateO2_peak[i] <- outputDF$reaRateSF6_peak[i] * reaRateConv #convert from SF6 to O2 reaeration rate coefficient
    outputDF$reaRateO2_nominal[i] <- outputDF$reaRateSF6_nominal[i] * reaRateConv #convert from SF6 to O2 reaeration rate coefficient
    outputDF$reaRateO2release_peak[i] <- outputDF$reaRateSF6release_peak[i] * reaRateConv #convert from SF6 to O2 reaeration rate coefficient
    outputDF$reaRateO2release_nominal[i] <- outputDF$reaRateSF6release_nominal[i] * reaRateConv #convert from SF6 to O2 reaeration rate coefficient
    ...
    #outputDF$meanQ[i] <- mean(inputFile[inputFile[[eventIDIdx]] == currEventID, QIdx], na.rm = T)*convLpsCms # m^3 s^-1
    #outputDF$meanDepth[i] <- outputDF$meanQ[i]/(inputFile[inputFile[[namLocIdx]] == S4 & inputFile[[eventIDIdx]] == currEventID, wwIdx]*outputDF$centroidVelocity[i]) # meters
    outputDF$gasTransVelO2_peak[i] <- outputDF$reaRateO2_peak[i] * outputDF$meanDepth[i] # d^-1 * m
    outputDF$gasTransVelO2_nominal[i] <- outputDF$reaRateO2_nominal[i] * outputDF$meanDepth[i] # d^-1 * m
    outputDF$gasTransVelO2release_peak[i] <- outputDF$reaRateO2release_peak[i] * outputDF$meanDepth[i] # d^-1 * m
    outputDF$gasTransVelO2release_nominal[i] <- outputDF$reaRateO2release_nominal[i] * outputDF$meanDepth[i] # d^-1 * m
    ...
    outputDF$k600_peak[i] <- (Sc_CO2/scO2)^(-0.5) * outputDF$gasTransVelO2_peak[i] #Equation 1, Wanninkhof (1992) "little k"
    outputDF$K600_peak <- outputDF$k600_peak[i] / outputDF$meanDepth[i] # d^-1 "Big K"
    
    outputDF$k600_nominal[i] <- (Sc_CO2/scO2)^(-0.5) * outputDF$gasTransVelO2_nominal[i] #Equation 1, Wanninkhof (1992) "little k"
    outputDF$K600_nominal <- outputDF$k600_nominal[i] / outputDF$meanDepth[i] # d^-1 "Big K"
    
    outputDF$k600release_peak[i] <- (Sc_CO2/scO2)^(-0.5) * outputDF$gasTransVelO2release_peak[i] #Equation 1, Wanninkhof (1992) "little k"
    outputDF$K600release_peak <- outputDF$k600release_peak[i] / outputDF$meanDepth[i] # d^-1 "Big K"
    
    outputDF$k600release_nominal[i] <- (Sc_CO2/scO2)^(-0.5) * outputDF$gasTransVelO2release_nominal[i] #Equation 1, Wanninkhof (1992) "little k"
    outputDF$K600release_nominal <- outputDF$k600release_nominal[i] / outputDF$meanDepth[i] # d^-1 "Big K"
