estimate_scaling_coefs <- function(dir = 'data/derived/hydraulics') {
  ...
  for(i in 1:length(files)) {
    ...
    dat <- readr::read_csv(files[i]) %>% 
      dplyr::filter(mean_velocity_ms > 0)
    
    site <- stringr::str_split(files[i], '/')[[1]][4] %>% 
      substr(.,1,4)
    ...
    lm_width <- lm(data = dat %>% 
                     dplyr::filter(is.finite(log(mean_width_m)),
                                   is.finite(log(calcQ_m3s))
                     ),
    ...
    plot <- dat %>% 
      dplyr::select(-est_mean_width_m) %>% 
      tidyr::pivot_longer(cols = 2:4,
                          names_to = 'meas',
                          values_to = 'val') %>% 
      ggplot2::ggplot(.,
                      ggplot2::aes(x = log(calcQ_m3s),
                                   y = log(val)))+
      ggplot2::geom_point()+
    ...
    ggplot2::ggsave(plot = plot,
                    glue::glue('figures/hydraulic_scaling/{site}_hydraulicScaling.png'),
                    height = 6, width = 4)
